...
class WeatherDirectTest < Minitest::Test
  def test_direct_list_weather
    setup = weather_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-weather", setup[:live] ? "live" : "unit")
    if _should_skip
...
def weather_direct_setup(mockres)
  ...
  if live
    ...
    client = GlaxWeatherSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
