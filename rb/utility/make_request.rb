...
module GlaxWeatherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GlaxWeatherResponse.new({})
    ...
    if fetch_err
      ...
      response = GlaxWeatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GlaxWeatherResponse.new(fetched)
    else
