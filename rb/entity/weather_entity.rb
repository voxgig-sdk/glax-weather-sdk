...
class WeatherEntity
  ...
  def make
    opts = @_entopts.dup
    WeatherEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GlaxWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Weather, Hash] the current Weather data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GlaxWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Weather fields)
  def match_get
  ...
  #
  # @param reqmatch [WeatherLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Weather.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Weather, Hash] the loaded Weather; raises GlaxWeatherError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GlaxWeatherHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [WeatherListMatch, Hash, nil] match filter (any subset of
  #   Weather fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Weather>, Array] the matching Weather items; raises GlaxWeatherError on failure
