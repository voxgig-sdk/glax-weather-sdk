...
class WeatherEntityTest extends TestCase
{
    ...
    {
        $testsdk = GlaxWeatherSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = GlaxWeatherSDK::test($seed, null);
        $seen = iterator_to_array($base->Weather(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = GlaxWeatherConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = GlaxWeatherSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GLAXWEATHER_TEST_WEATHER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function weather_basic_setup($extra)
{
    ...
    $client = GlaxWeatherSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("GLAXWEATHER_TEST_WEATHER_ENTID");
    ...
    if ($env["GLAXWEATHER_TEST_LIVE"] === "TRUE") {
        ...
        $client = new GlaxWeatherSDK(Helpers::to_map($merged_opts));
    }
