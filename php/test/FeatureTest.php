...
class FtCtrl extends GlaxWeatherControl
{
...
class FtClient
{
    ...
    private GlaxWeatherUtility $utility;

    public function __construct(GlaxWeatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): GlaxWeatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GlaxWeatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new GlaxWeatherUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (GlaxWeatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GlaxWeatherConfig::make_feature($name);
    ...
    private static function buildUrl(GlaxWeatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GlaxWeatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GlaxWeatherResponse($fetched);
        }
    ...
    private function fail(GlaxWeatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GlaxWeatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
