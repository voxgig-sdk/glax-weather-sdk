...
{
    private static function utility(): GlaxWeatherUtility
    {
        return new GlaxWeatherUtility();
    }
    ...
    private static function ctx(array $over = []): GlaxWeatherContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GlaxWeatherContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GlaxWeatherOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GlaxWeatherError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GlaxWeatherOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GlaxWeatherMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GlaxWeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GlaxWeatherMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GlaxWeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GlaxWeatherMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlaxWeatherSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GlaxWeatherMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GlaxWeatherResponse([]),
            'result' => new GlaxWeatherResult([]),
        ]);
        [, $err] = GlaxWeatherMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec([]),
            'response' => null,
            'result' => new GlaxWeatherResult([]),
        ]);
        [, $err] = GlaxWeatherMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec(['step' => 's']),
            'response' => new GlaxWeatherResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GlaxWeatherResult([]),
        ]);
        [, $err] = GlaxWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec(['step' => 's']),
            'response' => new GlaxWeatherResponse(self::resp(200, ['v' => 1])),
            'result' => new GlaxWeatherResult([]),
        ]);
        [, $err] = GlaxWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec(['step' => 's']),
            'response' => new GlaxWeatherResponse(self::resp(200, ['v' => 2])),
            'result' => new GlaxWeatherResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GlaxWeatherMakeResponse::call($ctx);
    ...
    {
        $preset = new GlaxWeatherResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec([]),
            'response' => new GlaxWeatherResponse([]),
            'result' => new GlaxWeatherResult([]),
        ...
        [$got, $err] = GlaxWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GlaxWeatherResult([])]);
        [, $err] = GlaxWeatherMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GlaxWeatherSpec([]), 'result' => null]);
        [, $err] = GlaxWeatherMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlaxWeatherSpec(['step' => 's']),
            'result' => new GlaxWeatherResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GlaxWeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlaxWeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GlaxWeatherSpec(['step' => 's']),
            'result' => new GlaxWeatherResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GlaxWeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GlaxWeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GlaxWeatherResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GlaxWeatherSpec([]),
            'result' => new GlaxWeatherResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GlaxWeatherMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlaxWeatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GlaxWeatherError('boom', 'boom');
        $utility->fetcher = function (GlaxWeatherContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlaxWeatherSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GlaxWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GlaxWeatherContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GlaxWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GlaxWeatherResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GlaxWeatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GlaxWeatherContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GlaxWeatherSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GlaxWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GlaxWeatherResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GlaxWeatherSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GlaxWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GlaxWeatherMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GlaxWeatherMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GlaxWeatherResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GlaxWeatherDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GlaxWeatherResult(['ok' => false])]);
        $this->expectException(GlaxWeatherError::class);
        GlaxWeatherDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GlaxWeatherResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GlaxWeatherMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GlaxWeatherMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GlaxWeatherBaseFeature();
        $b = new GlaxWeatherBaseFeature();
        GlaxWeatherFeatureAdd::call($ctx, $a);
        GlaxWeatherFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GlaxWeatherBaseFeature
    {
        $f = new GlaxWeatherBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GlaxWeatherFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GlaxWeatherContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GlaxWeatherSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GlaxWeatherPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GlaxWeatherPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GlaxWeatherResponse(['headers' => null]),
            'result' => new GlaxWeatherResult([]),
        ]);
        GlaxWeatherResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GlaxWeatherResultBody::call($ctx);
