...
class WeatherEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new WeatherEntity($this->_client, $opts);
    }

    /**
     * @param Weather|array $args Weather data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GlaxWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Weather|array The current Weather data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Weather fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GlaxWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Weather fields).
     */
    ...
    /**
     * Load a single Weather.
     *
     * @param WeatherLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed WeatherLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Weather|array The loaded Weather as an assoc-array at the
     *   SDK boundary; throws GlaxWeatherError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GlaxWeatherHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Weather items matching the given filter.
     *
     * @param WeatherListMatch|array|null $reqmatch Match filter (any subset
     *   of Weather fields) as an assoc-array; WeatherListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Weather[]|array A list of Weather items as assoc-arrays at
     *   the SDK boundary; throws GlaxWeatherError on failure (item-5 convention).
     */
