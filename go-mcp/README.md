...
[MCP](https://modelcontextprotocol.io) server exposing the GlaxWeather SDK as
two agent tools — `glax-weather_list` and `glax-weather_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/glax-weather-mcp)
make build

# 2. Provide credentials via the environment
export GLAX_WEATHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user glax-weather \
  -- /absolute/path/to/glax-weather-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./glax-weather-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// glax-weather_list: first page of records
{ "entity": "weather" }
{ "entity": "weather", "query": { } }

// glax-weather_load: one record by id
{ "entity": "weather", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/glax-weather-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user glax-weather \
     -- "$PWD"/dist/*/glax-weather-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `glax-weather_list` and `glax-weather_load` tools now appear
   in new sessions. Ask the agent to *"list weather using glax-weather"*
   and it calls `glax-weather_list` with `{"entity":"weather"}`.
...
```sh
export GLAX_WEATHER_APIKEY=sk_live_xxx            # API key
export GLAX_WEATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `glax-weather_list` tool
...
### Call the `glax-weather_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `glax-weather_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `glax-weather_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GLAX_WEATHER_APIKEY` | API key sent with every request. |
| `GLAX_WEATHER_BASE` | Optional override of the API base URL. |
...
```sh
./glax-weather-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"glax-weather_load","arguments":{"entity":"weather","query":{"id":1}}}}'
```
