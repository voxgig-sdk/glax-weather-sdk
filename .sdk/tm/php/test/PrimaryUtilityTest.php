...
// GlaxWeather SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        GlaxWeatherSDK $client,
        GlaxWeatherUtility $utility
    ): GlaxWeatherContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new GlaxWeatherContext($ctxmap, null);
    ...
    private static function fixctx(GlaxWeatherContext $ctx, GlaxWeatherSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?GlaxWeatherError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GlaxWeatherError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new GlaxWeatherResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new GlaxWeatherBaseFeature();
    ...
    {
        $hook_client = GlaxWeatherSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends GlaxWeatherBaseFeature {
            public $hook_fn = null;
            public function TestHook(GlaxWeatherContext $ctx): void
            {
    ...
    {
        $init_client = GlaxWeatherSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends GlaxWeatherBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new GlaxWeatherSDK([
            'system' => [
    ...
    {
        $blocked_client = new GlaxWeatherSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new GlaxWeatherSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = GlaxWeatherSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new GlaxWeatherOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = GlaxWeatherSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof GlaxWeatherError) ? $result->err->msg : (string)$result->err;
