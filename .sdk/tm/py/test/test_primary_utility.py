...
from projectname_sdk import GlaxWeatherSDK
from core.spec import GlaxWeatherSpec
from core.result import GlaxWeatherResult
from core.response import GlaxWeatherResponse
from core.operation import GlaxWeatherOperation
from core.error import GlaxWeatherError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return GlaxWeatherError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = GlaxWeatherSDK.test(None, None)
        ...
        class TestInitFeature(GlaxWeatherBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(GlaxWeatherBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = GlaxWeatherSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = GlaxWeatherSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = GlaxWeatherOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = GlaxWeatherSDK.test(None, setup_opts)
