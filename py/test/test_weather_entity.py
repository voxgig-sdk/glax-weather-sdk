...
class TestWeatherEntity:
    ...
    def test_should_stream(self):
        ...
        base = GlaxWeatherSDK.test(seed, None)
        seen = list(base.Weather(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = GlaxWeatherSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Weather(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GLAXWEATHER_TEST_WEATHER_ENTID JSON to run live")
        client = setup["client"]
...
def _weather_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GLAXWEATHER_TEST_WEATHER_ENTID"))
    ...
    if env.get("GLAXWEATHER_TEST_LIVE") == "TRUE":
        ...
        client = GlaxWeatherSDK(helpers.to_map(merged_opts))

    _live = env.get("GLAXWEATHER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("GLAXWEATHER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
