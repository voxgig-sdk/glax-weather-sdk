...
local WeatherEntity = {}
WeatherEntity.__index = WeatherEntity


function WeatherEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WeatherEntity)
  self._name = "weather"
  self._client = client
...
function WeatherEntity:get_name()
  return self._name
end


function WeatherEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WeatherEntity.new(self._client, opts)
end


function WeatherEntity:data_set(args)
  if args ~= nil then
...
function WeatherEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WeatherEntity:match_set(args)
  if args ~= nil then
...
function WeatherEntity:match_get()
...
function WeatherEntity:stream(action, args, callopts)
...
---@param reqmatch WeatherLoadMatch
---@param ctrl? table
---@return Weather
---@return string? err
function WeatherEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WeatherEntity:_run_op(ctx, post_done)
  local utility = self._utility
