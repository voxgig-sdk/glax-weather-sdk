...
AQL-driven command-line client **and** interactive REPL for the GlaxWeather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/glax-weather-cli)
make build

# 2. See usage (words, entities, env vars)
./glax-weather-cli --help
...
./glax-weather-cli list weather
./glax-weather-cli load 1 weather            # {id:1} shorthand
./glax-weather-cli load '{id:1}' weather       # explicit match map

# 5. Override the API base URL for a single call
GLAX_WEATHER_BASE=https://api.example.com ./glax-weather-cli list weather

# 6. No arguments -> interactive REPL
./glax-weather-cli
glax-weather> list weather
glax-weather> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/glax-weather-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/glax-weather-cli list weather
   ```
...
```sh
./glax-weather-cli load 1 weather          # scalar shorthand for {id:1}
./glax-weather-cli load '{id:1}' weather     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `glax-weather>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `weather`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/glax-weather-cli`. |
...
(`./glax-weather-cli <expr>`) and interactively (the REPL), and expressions compose the
