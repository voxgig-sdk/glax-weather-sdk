...
describe('WeatherDirect', async () => {
  ...
  test('direct-load-weather', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-weather', setup.live)) return
  ...
  test('direct-list-weather', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-weather', setup.live)) return
    const { client, calls } = setup
