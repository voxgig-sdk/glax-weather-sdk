...
//     (\`new GlaxWeatherSDK(...)\` / \`GlaxWeatherSDK.test(...)\` become
//     \`GlaxWeatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"weather":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GlaxWeatherSDK(...)\` and any
// \`GlaxWeatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GlaxWeatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GlaxWeatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
